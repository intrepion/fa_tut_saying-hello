...
describe('HttpGreetingApi', () => {
  ...
  it('requests the canonical greeting endpoint', async () => {
    ...
    const api = new HttpGreetingApi('http://localhost:25664');
    const result = await api.getGreeting('Ada');

    expect(fetchMock).toHaveBeenCalledWith('http://localhost:25664/api/greeting?name=Ada');
...
describe('HttpGreetingApi', () => {
  ...
  it('omits the query string for empty input', async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith('http://localhost:25664/api/greeting');
...
describe('index page', () => {
  it('renders the greeting form and API endpoint', async () => {
    ...
    expect(result).toContain('data-api-base-url="http://localhost:25664"');
  });
});
```
...
```astro
---
const apiBaseUrl = 'http://localhost:25664';
---
